...
info.controller('info', function($scope, $http) {
  ...
  $scope.hideCancelRide=true;
  $scope.hideEditRide=true;
 ...
 // alert("in controller");
  ...
  $http({
      ...
      $scope.hideCancelRide=false;
      $scope.hideEditRide=false;
...
info.controller('AppCtrl', function($scope, $http) {
	$scope.hide_trips = true;
	$scope.hideProfile = true;
	$scope.showBill=true;
	...
	$scope.endRide = function(){
		console.log(" I am in endRide controller");
		//$scope.ridehistory = true;
			$scope.generateBill= false;
			$scope.hide_trips=false;
			console.log(response);
