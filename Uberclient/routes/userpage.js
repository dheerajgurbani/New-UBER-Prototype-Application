		...
		console.log("other outside extra"+results.uid);
		   ...
		   console.log("outside"+results.data);
			if(results.code == 200){
				...
				req.session.ufname=results.ufname;
...
exports.confirmRide = function(req,res){
			...
			var msg_payload = { "did": did,"uid":req.session.uid,"ufname":req.session.ufname,"source1":source1,"destination":destination, "uemail":req.session.email,"task": task};
